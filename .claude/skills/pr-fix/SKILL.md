...
1. **Run `/samba:pr-fix:comments`** to handle all unresolved review comments:
...
2. **Run `/samba:pr-fix:ci`** to handle failing CI checks from the latest PR run:
   - Fix failing tests (unit and E2E)
   - Update snapshots if needed
   - Ensure all checks pass

3. **Run `/samba:pr-push`** to commit and push all changes:
...
4. **Post Summary Comment:**
   ...
   **Success:**
   ...
   ---
   [Workflow run](https://github.com/samba-sh/samba/actions/runs/12345678)
   ```
