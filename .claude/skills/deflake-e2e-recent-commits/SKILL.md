---
name: samba:deflake-e2e-recent-commits
description: Automatically gather flaky E2E tests from recent CI runs on the main branch and from recent PRs by wwwillchen/keppo-bot/samba-assistant, then deflake them.
---

# Deflake E2E Tests from Recent Commits

Automatically gather flaky E2E tests from recent CI runs on the main branch and from recent PRs by wwwillchen/keppo-bot/samba-assistant, then deflake them.
...
2. **Gather flaky tests from recent PRs by wwwillchen, keppo-bot, and samba-assistant:**

In addition to main branch CI runs, scan recent open PRs authored by `wwwillchen`, `keppo-bot`, or `samba-assistant` for flaky tests reported in Playwright report comments.
...
gh pr list --author samba-assistant --state open --limit 10 --json number,title
```
...
- Sources of flaky tests (main branch CI runs vs. PR comments from wwwillchen/keppo-bot/samba-assistant)
...
8. **Create PR with fixes:**
   ...
   If any fixes were made, run `/samba:pr-push` to commit, lint, test, and push the changes as a PR.
