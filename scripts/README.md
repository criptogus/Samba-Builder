...
1. `samba-{version}-1.x86_64.rpm` (Linux RPM)
2. `samba-{version}-full.nupkg` (Windows NuGet package)
3. `samba-{version}.Setup.exe` (Windows installer)
4. `samba-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `samba-darwin-x64-{version}.zip` (macOS Intel)
6. `samba_{version}_amd64.deb` (Linux DEB)
