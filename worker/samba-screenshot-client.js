(() => {
  ...
  function getScreenshotDimensions() {
    ...
    console.warn(
      `[samba-screenshot] Full page ${fullPage.width}x${fullPage.height} exceeds the screenshot memory budget; capturing viewport ${boundedViewport.width}x${boundedViewport.height} instead.`,
    );
    return {
      ...boundedViewport,
      fullPage,
  ...
  async function captureScreenshotOnce() {
    try {
      ...
      console.error("[samba-screenshot] Failed to capture screenshot:", error);
      throw error;
    }
  }
  ...
  async function handleScreenshotRequest(requestId) {
    try {
      console.debug("[samba-screenshot] Capturing screenshot...");

      const dataUrl = await captureScreenshot();

      console.debug("[samba-screenshot] Screenshot captured successfully");

      // Send success response to parent
      window.parent.postMessage(
        {
          type: "samba-screenshot-response",
          requestId,
          success: true,
          dataUrl: dataUrl,
        },
      ...
      console.error("[samba-screenshot] Screenshot capture failed:", error);
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.type === "samba-take-screenshot") {
