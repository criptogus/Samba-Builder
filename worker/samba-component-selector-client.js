(() => {
  const OVERLAY_CLASS = "__samba_overlay__";
  let overlays = [];
  ...
  function updateOverlay(el, isSelected = false, isHighlighted = false) {
    ...
    const name = el.dataset.sambaName || "<unknown>";
    const file = (el.dataset.sambaId || "").split(":")[0];
    ...
    if (highlightedElement) {
      ...
      if (highlightedItem) {
        ...
        window.parent.postMessage(
          {
            type: "samba-component-coordinates-updated",
            coordinates: {
              top: rect.top,
              left: rect.left,
              width: rect.width,
  ...
  function removeOverlayById(componentId) {
    ...
    overlays.forEach((item, index) => {
      if (item.el.dataset.sambaId === componentId) {
    ...
    if (
      highlightedElement &&
      highlightedElement.dataset.sambaId === componentId
    ) {
      highlightedElement = null;
    }
  }
   ...
   * Returns the parent samba-tagged element if the current one is an overlay,
  ...
  function skipOverlayElement(el) {
    ...
    // This looks like an overlay — walk up to the parent with a samba-id
    let parent = el.parentElement;
    while (parent && !parent.dataset.sambaId) parent = parent.parentElement;
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.sambaId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    const clickedComponentId = state.element.dataset.sambaId;
    ...
    if (selectedItem && (highlightedElement === state.element || !isProMode)) {
      ...
      window.parent.postMessage(
        {
          type: "samba-component-deselected",
    ...
    if (!state.element.dataset.sambaRuntimeId) {
      state.element.dataset.sambaRuntimeId = `samba-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }

    const rect = state.element.getBoundingClientRect();
    window.parent.postMessage(
      {
        type: "samba-component-selected",
        component: {
          id: clickedComponentId,
          name: state.element.dataset.sambaName,
          runtimeId: state.element.dataset.sambaRuntimeId,
        },
        coordinates: {
          top: rect.top,
          left: rect.left,
  ...
  function onKeyDown(e) {
    ...
    if (key === "r" && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "samba-preview-reload-shortcut",
        },
        "*",
      );
      return;
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "samba-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (
      ...
      e.data?.type === "samba-preview-reload-shortcut"
    ) {
      window.parent.postMessage(e.data, "*");
      return;
    }
    if (e.source !== window.parent) return;
    if (e.data.type === "samba-pro-mode") {
      isProMode = e.data.enabled;
    }
    if (e.data.type === "activate-samba-component-selector") activate();
    if (e.data.type === "deactivate-samba-component-selector") deactivate();
    if (e.data.type === "activate-samba-visual-editing") {
      activate();
    }
    if (e.data.type === "deactivate-samba-visual-editing") {
      deactivate();
      clearOverlays();
    }
    if (e.data.type === "clear-samba-component-overlays") clearOverlays();
    if (e.data.type === "update-samba-overlay-positions") {
    ...
    if (
      e.data.type === "remove-samba-component-overlay" ||
      e.data.type === "deselect-samba-component"
    ) {
      if (e.data.componentId) {
        removeOverlayById(e.data.componentId);
      }
    }
    if (e.data.type === "restore-samba-component-overlays") {
      const componentIds = e.data.componentIds;
      if (Array.isArray(componentIds)) {
        clearOverlays();
        for (const id of componentIds) {
          const el = document.querySelector(
            `[data-samba-id="${CSS.escape(id)}"]`,
          );
          if (el) {
  ...
  function initializeComponentSelector() {
    ...
    function checkForTaggedElements() {
      if (document.body.querySelector("[data-samba-id]")) {
        ...
        window.parent.postMessage(
          {
            type: "samba-component-selector-initialized",
          },
          "*",
        );
    ...
    setTimeout(() => {
      ...
      observer = new MutationObserver((mutations) => {
        ...
        const hasRelevantMutation = mutations.some((mutation) => {
          // Attribute mutation on data-samba-id (already filtered by attributeFilter)
          if (mutation.type === "attributes") {
            return true;
          }
          // Check if any added nodes have data-samba-id
          if (mutation.type === "childList") {
            for (const node of mutation.addedNodes) {
              if (node.nodeType === Node.ELEMENT_NODE) {
                if (
                  node.hasAttribute("data-samba-id") ||
                  node.querySelector("[data-samba-id]")
                ) {
                  return true;
                }
              }
