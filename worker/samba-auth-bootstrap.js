 ...
 *   down (from parent): { type: "samba-auth-login", auth, nonce, token }
 *   up   (to parent):   { type: "samba-auth-bootstrap-ready", pendingNonce? }
 *                       { type: "samba-auth-ready", ok: boolean, error?: string,
 *                         path?: string }
 *
...
(() => {
  ...
  const authBootstrapToken = document.currentScript?.dataset.sambaAuthToken;
  try {
    document.currentScript?.removeAttribute("data-samba-auth-token");
  } catch {
    // Best effort: the value is already snapshotted above either way.
  }
  const PENDING_KEY = "__samba_auth_pending__";
  ...
  function post(ok, error, path) {
    window.parent.postMessage(
      { type: "samba-auth-ready", ok, error, path, nonce: activeNonce },
      "*",
    );
  }
  ...
  function checkPendingOnLoad() {
    ...
    // parent resends `samba-auth-login` in response.
    window.parent.postMessage(
      {
