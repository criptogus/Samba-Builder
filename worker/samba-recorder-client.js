 ...
 *
 * Protocol:
 *   down (from parent): { type: "activate-samba-recorder" | "deactivate-samba-recorder", token }
 *                       { type: "flush-samba-recorder", token, requestId }
 *   up   (to parent):   { type: "samba-recorder-initialized" }
 *                       { type: "samba-recorder-action", action: RecordedAction }
 *                       { type: "samba-recorder-flushed", requestId }
 *
...
(() => {
  ...
  const allowUntrustedEvents =
    window.__SAMBA_RECORDER_ALLOW_UNTRUSTED__ === true;
  ...
  const recorderToken = document.currentScript?.dataset.sambaRecorderToken;
  try {
    document.currentScript?.removeAttribute("data-samba-recorder-token");
  } catch {
    // Best effort: the value is already snapshotted above either way.
  }
  try {
    delete window.__SAMBA_RECORDER_ALLOW_UNTRUSTED__;
  } catch {
  ...
  const OVERLAY_CLASS = "__samba_recorder_overlay__";
  ...
  function buildCandidates(el) {
    ...
    // `data-samba-id` is deliberately NOT a candidate: it encodes a source
   ...
   * generated Playwright test never locates by `data-samba-id`.
   */
  function sourceHintFor(el) {
    ...
    while (source && source.nodeType === 1) {
      const raw = source.getAttribute && source.getAttribute("data-samba-id");
      if (
        raw &&
  ...
  function emit(action) {
    ...
    lastEmit = { key, at: now };
    window.parent.postMessage({ type: "samba-recorder-action", action }, "*");
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (type === "activate-samba-recorder") activate();
    else if (type === "deactivate-samba-recorder") deactivate();
    else if (
      type === "flush-samba-recorder" &&
      typeof e.data.requestId === "string"
    ) {
      window.parent.postMessage(
        { type: "samba-recorder-flushed", requestId: e.data.requestId },
        "*",
      );
    }
  });
  ...
  function init() {
    window.parent.postMessage({ type: "samba-recorder-initialized" }, "*");
