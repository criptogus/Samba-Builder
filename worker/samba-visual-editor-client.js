(() => {
  ...
  function findElementBySambaId(sambaId, runtimeId) {
    ...
    // Fall back to finding by samba-id (will get first match)
    const escaped = CSS.escape(sambaId);
    return document.querySelector(`[data-samba-id="${escaped}"]`);
  }
  ...
  function applyStyles(element, styles) {
    ...
    console.debug(
      `[Samba Builder Visual Editor] Applying styles:`,
      styles,
      "to element:",
      element,
    );
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementBySambaId(elementId, runtimeId);
    if (element) {
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementBySambaId(componentId, runtimeId);
    if (element) {
  ...
  function handleModifyImageSrc(data) {
    ...
    if (imgEl) {
      ...
      if (imgEl._sambaAbort) imgEl._sambaAbort.abort();
      const controller = new AbortController();
      imgEl._sambaAbort = controller;
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-samba-component-styles":
        handleGetStyles(data);
        break;
      case "modify-samba-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-samba-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-samba-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-samba-text-content":
        handleGetTextContent(data);
        break;
      case "modify-samba-image-src":
