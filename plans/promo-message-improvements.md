...
- Pro promo clicks open the in-app `SambaProTrialDialog` (no browser context switch); the dialog now takes a `utmCampaign` prop so checkout/learn-more traffic is attributed per message (`streaming-promo-<messageId>`).
...
- Shown only while `isStreaming` to non-Pro users (`!settings?.enableSambaPro && !userBudget`), then **disappears the moment the stream ends** — exactly when the user's attention returns to the chat.
...
- Messaging is feature-jargon-led ("Turbo Edits", "Smart Context") with no offer. Meanwhile the app _has_ a 7-day free trial (`trialCode=7PRO30`, used by SambaProTrialDialog/SetupBanner) that none of these messages mention.
...
4. **No offer, no proof, no numbers**: "Get Samba Pro for faster edits with Turbo Edits" means nothing to someone who doesn't know what Turbo Edits is.
...
- Promo click → open existing `SambaProTrialDialog` (no context switch, stream keeps going).
...
- Lead with the trial: "Try Samba Pro free for 7 days" beats "Get Samba Pro".
...
- Sparkles/gradient accent consistent with SambaProTrialDialog, real button-styled CTA, dark-mode support, subtle fade-in.
...
## Link destination: samba.sh/pro vs academy sign-in→checkout

- **Education-style messages** (user may not know what Pro is) → `https://www.samba.sh/pro?utm_...` — consideration page, low bounce.
- **Direct-offer messages** ("Start your free 7-day trial") → `https://academy.samba.sh/redirect-to-checkout?trialCode=7PRO30&utm_...` — same pattern the app already uses. Prefer this over hardcoding `/sign-in?redirect_url=...`: the redirect endpoint owns the auth flow (survives auth changes) and carries the trial code.
- **Best of both**: click → in-app `SambaProTrialDialog`, which offers "Start Free Trial" (checkout) and "Learn more" (/pro). Warm the user before the sign-in wall.
