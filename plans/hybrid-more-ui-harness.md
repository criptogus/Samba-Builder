...
| Settings / dialog pages        | `telemetry`, `max_tool_call_steps`, `smart_context_options`, `turbo_edits_options`, `samba_pro_key_validation`, `supabase_migrations` (toggle) | settings page sections / standalone settings components + `/settings/providers/$provider`                   |
...
| `local_agent_grep.spec.ts`     | samba-grep card content/clicks; convert aria snapshots to text assertions                                     | —                                               |
...
| `streaming_renderer.spec.ts`   | streamed samba-write block rendering + pending indicators                                                     | —                                               |
...
params, and `.samba/media` persistence on the real production path. The harness
...
| `smart_context_options`        | `ProModeSelector` standalone (it's a Popover off the chat input, so it can also be driven from the existing `/chat` mount) | seed pro key + `enableSambaPro: true` or buttons are disabled; assert `enableProSmartFilesContextMode` / `proSmartContextOption`                                                                                                                                                                                                        |
| `turbo_edits_options`          | same `ProModeSelector` mount                                                                                               | assert `enableProLazyEditsMode` / `proLazyEditsMode` (`v1`/`v2`)                                                                                                                                                                                                                                                                        |
| `samba_pro_key_validation`     | `/settings/providers/$provider` route with `provider: "auto"`                                                              | **no IPC mocking**: pass `engine: true` (existing option) so the real `validate-provider-api-key` handler streams against the fake server, which 401s keys matching `/invalid/i` → real "API key rejected" AlertDialog; happy path with `testsambakey`. The ApiKeyConfiguration "Paste" button (`navigator.clipboard`) is not exercised |
...
`samba_pro_key_validation`, and `supabase_migrations` e2e specs. Covered in
