...
- Samba Engine endpoints for sandbox provisioning (authenticates Pro user, manages Vercel credentials)
- Auto-hibernate sandboxes after 15 minutes of inactivity
- Maximum 1 concurrent cloud sandbox per user (v1)
- Stale sandbox reconciliation on Samba launch (clean up orphans from previous sessions)
...
1. **As a new Samba user**, I want to preview my AI-generated app without installing Node.js, so that I can start building immediately after downloading Samba.
2. **As a Samba user on a slow machine**, I want to offload app execution to the cloud, so that my laptop doesn't slow down while building.
3. **As a Samba Pro user**, I want to switch between local and cloud runtime with one click in Settings, so that I can choose the mode that works best for my situation.
...
2. **Selection**: User selects "Cloud Sandbox (Pro)" from the dropdown.
   - If **not Pro**: Inline upgrade prompt ("Cloud sandboxes are a Samba Pro feature. [Upgrade to Pro]"). No further action possible.
...
4. **Provisioning**: System creates a Vercel sandbox via Samba Engine. Loading state: "Starting cloud sandbox..." with a simple spinner. If provisioning takes >5 seconds, show a multi-step stepper: "Creating sandbox..." -> "Installing dependencies..." -> "Starting server..."
...
- **Error - Auth/Backend**: "Samba cloud services are temporarily unavailable. Switch to local mode to keep building. [Switch to Local]"
...
executeApp() decision point (app_handlers.ts)
    ...
    └── "cloud"  → executeAppInCloud() [NEW]
                ...
                Local Proxy Server (start_proxy_server.ts)
                      |  (injects Samba scripts, serves to iframe)
                      v
...
- **`src/ipc/utils/start_proxy_server.ts`** — Support proxying to remote HTTPS URLs (currently targets localhost only). Handle SSL, inject Samba scripts into response HTML.
- **`src/components/RuntimeModeSelector.tsx`** — Add "Cloud Sandbox (Pro)" option. Pro gate with upgrade prompt for free users.
- **`src/hooks/useRunApp.ts`** — Handle cloud preview URL delivery. Emit synthetic `app:output` with `[samba-proxy-server]` format for consistency.
...
- **New: `src/ipc/utils/cloud_sandbox_provider.ts`** — Provider interface and Vercel implementation:
  ...
  ```
- **New: Samba Engine endpoints** — Sandbox CRUD (create, destroy, status). Authenticates Pro subscription, manages Vercel credentials server-side.

### Data Model Changes

**MVP: No database schema changes.** Sandbox state is stored in-memory in the `runningApps` Map. Sandboxes are ephemeral and don't survive Samba restarts. On restart, the user simply re-runs the app (creates a new sandbox).
...
**New Samba Engine API endpoints:**
...
### Phase 2: Samba Backend Endpoints

- [ ] Determine if existing Samba Pro backend can be extended (or if new infrastructure is needed)
...
- [ ] Implement Vercel Sandbox SDK provider against the Samba Engine endpoints
...
- [ ] Implement graceful sandbox teardown on Samba quit (iterate `runningApps`, destroy cloud entries)
- [ ] Implement stale sandbox reconciliation on Samba startup
...
| ----------------------------------------------------------------------------- | ---------- | ------ | -------------------------------------------------------------------------------------------------------------------- |
| Samba Engine dependency — if Engine is down, cloud sandboxes are broken       | Medium     | High   | Monitoring, clear error messages ("Samba cloud services temporarily unavailable"), fallback to local mode suggestion |
| Orphaned sandboxes consuming Samba's cloud resources                          | High       | High   | Auto-hibernate after 15 min, max 1 concurrent sandbox, startup reconciliation, graceful teardown on quit             |
...
- **Samba Engine integration**: Sandbox management will be implemented in Samba Engine (the existing backend service). Specific endpoint design and Vercel SDK integration details need to be worked out.
...
| **Samba Pro feature** (Samba manages infrastructure) | Simplifies UX (no Vercel account setup needed), controls costs, creates Pro upsell opportunity. Requires Samba Engine for sandbox management.                                                           |
...
| **Auto-hibernate after 15 min**                      | Prevents orphaned sandboxes from running up Samba's cloud costs. Balance between convenience and cost control.                                                                                          |
