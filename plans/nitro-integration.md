...
Add a server layer (Nitro) to Vite/React apps in Samba through an **AI-callable tool** that the agent invokes automatically when the user's prompt indicates server-side needs (database access, secrets, API routes, webhooks). This closes the architectural gap between Next.js (which has built-in API routes) and Vite for database-backed apps — specifically unblocking Neon database integration for Vite projects, which requires server-side access to `DATABASE_URL`. Nitro-specific coding conventions are appended to the project's `AI_RULES.md` so all future chats know the patterns.

## Problem Statement

Samba users building React/Vite apps have no server layer. This creates three concrete problems:
...
Since Vite is the default template, this affects the majority of Samba apps.
...
4. Tool execution renders a chat-visible XML tag (e.g., `<samba-enable-nitro />`) similar to `<samba-add-integration>`, so the user sees the scaffolding action in the message stream
...
`enable_nitro` is **not registered in legacy build mode**. Build mode uses the text-stream + `samba-write` XML-tag pipeline ([chat_stream_handlers.ts:1330](src/ipc/handlers/chat_stream_handlers.ts#L1330)), not real model tool calls. Two options for build-mode users in v1:
...
- **Option B (deferred)**: Add a parallel build-mode path that scaffolds Nitro via a `<samba-enable-nitro />` XML tag the way `samba-add-integration` already works. Skipped because it duplicates logic and build mode is being phased toward local-agent.
...
- **Tool-call message in chat** — Renders like other agent tool calls (consistent with existing `samba-write` / MCP tool UX) so the user sees what's being scaffolded.
...
| **AI Tool**             | `src/pro/main/ipc/handlers/local_agent/tools/enable_nitro.ts` (new)                              | `enable_nitro` tool definition matching the existing `ToolDefinition` shape (mirrors `add_integration.ts` for signaling, `add_dependency.ts` for the install flow). Fields: `name`, `description` (carries post-call setup steps), `inputSchema`, `defaultConsent: "always"`, `modifiesState: true`, `isEnabled: (ctx) => ctx.frameworkType === "vite" && !ctx.nitroEnabled`, `getConsentPreview`, `buildXml` (renders `<samba-enable-nitro />`), `execute` (the scaffolding logic) |
...
| **Chat UI renderer**    | `src/components/chat/SambaMarkdownParser.tsx` + `src/components/chat/SambaEnableNitro.tsx` (new) | Add `samba-enable-nitro` to the `SAMBA_CUSTOM_TAGS` allowlist and a `case "samba-enable-nitro":` in `renderCustomTag` (mirror the `samba-add-integration` case). Render a minimal status card — no interactive UI; the tool's `execute` already did the scaffolding.                                                                                                                                                                                                                |
...
Example final vite.config.ts:
     ...
     export default defineConfig({
       plugins: [sambaComponentTagger(), react(), nitro()],
     });
...
3. Then write the user-requested API route(s) following the conventions documented
  ...
  buildXml: (_args, _isComplete) => `<samba-enable-nitro />`,
...
- [ ] `buildXml: () => "<samba-enable-nitro />"` so the action renders in the chat stream
...
- [ ] Add `samba-enable-nitro` to the `SAMBA_CUSTOM_TAGS` allowlist in `src/components/chat/SambaMarkdownParser.tsx` and a `case "samba-enable-nitro":` in `renderCustomTag` (mirror the `samba-add-integration` case); create `src/components/chat/SambaEnableNitro.tsx` for the minimal status-card render
...
- [ ] Verify `sambaComponentTagger` plugin doesn't conflict with Nitro plugin (rules dictate Nitro-last ordering so Vite's module-transform middleware runs before Nitro's SPA fallback)
...
| `sambaComponentTagger` plugin conflicts with Nitro plugin                             | Low        | Low    | AI rules dictate Nitro-last ordering in the plugins array so Vite's module-transform middleware handles `/src/*`, `/@vite/*`, `/@fs/*` before Nitro's SPA fallback                                        |
...
- **`DATABASE_URL` injection during local dev** (follow-up): When Neon is connected, how does the dev server get the connection string? Options: Samba writes `.env` file, Samba sets env var on process, or system prompt instructs agent. Resolve before Phase 5.
...
| **Local-agent mode only (build mode gets a switch-mode nudge)**        | Real model-dispatched tool calls only flow through `handleLocalAgentStream`; legacy build mode uses text-stream + `samba-write` XML tags. Adding a parallel build-mode XML scaffolder duplicates logic for a code path that's being phased out. A prompt-level nudge in build mode keeps users informed without forking the implementation. |
