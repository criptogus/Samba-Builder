    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ? JSON.stringify(message.content)
            // Normalize attachment paths (dynamic MD5 hashes in .samba/media)
            .replace(
              /path: [^"]*?[/\\]{1,2}\.samba[/\\]{1,2}media[/\\]{1,2}[a-f0-9]+\.\w+/g,
              "path: [[ATTACHMENT_PATH]]",
            )
            // Also normalize .samba/media paths in escaped attribute format
            .replace(
              /[/\\]{1,2}\.samba[/\\]{1,2}media[/\\]{1,2}[a-f0-9]{6,}\.\w+/g,
              "/.samba/media/[[ATTACHMENT_HASH]]",
            )
        : message.content
            ...
            .replace(
              /\n<samba-file path="package\.json">[\s\S]*?<\/samba-file>\n/g,
              "",
            );
