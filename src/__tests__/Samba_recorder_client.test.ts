...
const recorderSource = fs.readFileSync(
  path.resolve(process.cwd(), "worker/samba-recorder-client.js"),
  "utf8",
);
...
function setup({ allowUntrusted = true }: { allowUntrusted?: boolean } = {}) {
  ...
  Object.defineProperty(doc, "currentScript", {
    configurable: true,
    value: {
      dataset: { sambaRecorderToken: RECORDER_TOKEN },
  ...
  const parent = {
    postMessage: (msg: any) => {
      messages.push(msg);
      if (msg && msg.type === "samba-recorder-action") actions.push(msg.action);
    },
  };
  ...
  const win: any = {
    ...
    __SAMBA_RECORDER_ALLOW_UNTRUSTED__: allowUntrusted,
  ...
  if (!messages.some((m) => m.type === "samba-recorder-initialized")) {
  ...
  const activate = (token = RECORDER_TOKEN) =>
    messageHandler!({
      source: parent,
      data: { type: "activate-samba-recorder", token },
    });
  const deactivate = () =>
    messageHandler!({
      source: parent,
      data: { type: "deactivate-samba-recorder", token: RECORDER_TOKEN },
    });
  const flush = (requestId: string) =>
    messageHandler!({
      source: parent,
      data: {
        type: "flush-samba-recorder",
  ...
  const overlays = () => doc.querySelectorAll(".__samba_recorder_overlay__");
...
describe("samba recorder client", () => {
  ...
  it("takes the capability out of the DOM once it has read it", () => {
    ...
    expect(r.removedAttributes).toContain("data-samba-recorder-token");
  ...
  it("acknowledges a flush after actions already sent to the parent", () => {
    ...
    expect(r.messages.slice(-2)).toEqual([
      expect.objectContaining({ type: "samba-recorder-action" }),
      { type: "samba-recorder-flushed", requestId: "flush-1" },
    ]);
  });
  ...
  it("uses data-samba-id only as a source hint for a CSS fallback", () => {
    const r = setup();
    ...
    // doesn't even carry.
    r.setHtml(`<div data-samba-id="src/App.tsx:12:4"><span></span></div>`);
    r.activate();
    r.click(r.doc.querySelector("span"));
    r.settleClick();
  ...
  it("captures the exact date input source for selector stabilization", () => {
    const r = setup();
    r.setHtml(
      `<main><input type="date" data-samba-id="src/EventForm.tsx:84:10" /></main>`,
    );
    r.activate();
  ...
  it("accepts a source path at the schema's 2,048-character limit", () => {
    ...
    r.setHtml(
      `<main><input type="date" data-samba-id="${relativePath}:1:0" /></main>`,
    );
    r.activate();
    r.typeInto(r.doc.querySelector("input"), "2026-08-13");
  ...
  it("cannot be switched into accepting untrusted events after load", () => {
    ...
    const r = setup({ allowUntrusted: false });
    r.win.__SAMBA_RECORDER_ALLOW_UNTRUSTED__ = true;
    r.setHtml(`<button>Add</button>`);
    r.activate();
    r.click(r.doc.querySelector("button"));
    r.settleClick();
  ...
  it("removes the escape-hatch global once it has been read", () => {
    const r = setup();
    expect(r.win.__SAMBA_RECORDER_ALLOW_UNTRUSTED__).toBeUndefined();
  });
});
