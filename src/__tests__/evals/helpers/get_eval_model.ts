...
// because Samba Builder's production picker does not currently surface GPT 5.4 —
 ...
 *
 * The Samba Builder Engine only supports `stream: true`, but the AI SDK sends
...
/**
 * Fetch wrapper that adapts requests for the Samba Builder Engine, which only supports
...
const evalFetch: typeof fetch = async (input, init) => {
  ...
  // Force streaming — the Samba Builder Engine returns 500 for non-streaming requests
  parsed.stream = true;
...
export function getEvalModel(
  ...
  // 1. The Samba Builder Engine only supports streaming for chat completions, and the
