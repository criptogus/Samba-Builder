...
const bootstrapSource = fs.readFileSync(
  path.resolve(process.cwd(), "worker/samba-auth-bootstrap.js"),
  "utf8",
);
...
function setup({
  ...
  const sessionStorage = makeStorage(
    pending
      ? {
          ...
          __samba_auth_pending__: JSON.stringify({
            startedAt: Date.now(),
            ...pending,
          }),
        }
  ...
  const document = {
    readyState,
    currentScript: {
      dataset: { sambaAuthToken: authBootstrapToken },
    },
    addEventListener: () => {},
  };
  ...
  return {
    ...
    sendLogin: (
      ...
      messageHandler?.({
        source: parent,
        data: { type: "samba-auth-login", auth, nonce, token },
      }),
  };
}
...
describe("samba auth bootstrap", () => {
  it("seeds the supabase session into localStorage and lands on the homepage", async () => {
    ...
    const marker = JSON.parse(
      h.sessionStorage.getItem("__samba_auth_pending__")!,
    );
  ...
  it("announces bootstrap-ready on a fresh (pre-login) load", () => {
    const h = setup();
    expect(h.posts.some((p) => p.type === "samba-auth-bootstrap-ready")).toBe(
      true,
    );
  });
  ...
  it("ignores login messages from a framing parent without the proxy capability", () => {
    ...
    expect(h.sessionStorage.getItem("__samba_auth_pending__")).toBeNull();
  });
  ...
  it("reports failure (and does not navigate) when sign-in fails", async () => {
    ...
    await vi.waitFor(() =>
      expect(
        h.posts.some((p) => p.type === "samba-auth-ready" && p.ok === false),
      ).toBe(true),
    );
    expect(h.replace).not.toHaveBeenCalled();
  });
  ...
  it("verifies a pending supabase session once the parent claims the attempt", async () => {
    ...
    expect(h.posts).toContainEqual(
      expect.objectContaining({
        type: "samba-auth-bootstrap-ready",
    ...
    expect(
      h.posts.some((p) => p.type === "samba-auth-ready" && p.ok === true),
    ).toBe(false);
  ...
  it("returns to / when the app auth guard redirects during session startup", async () => {
    ...
    expect(
      JSON.parse(h.sessionStorage.getItem("__samba_auth_pending__")!),
    ).toMatchObject({
      mode: "supabase-password",
      ref: "ref123",
      homeRedirects: 1,
