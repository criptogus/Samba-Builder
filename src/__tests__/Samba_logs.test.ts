...
const SAMBA_LOGS_SCRIPT = readFileSync(
  resolve(__dirname, "../../worker/samba_logs.js"),
  "utf8",
);
...
function loadConsoleInterceptor() {
  ...
  runInNewContext(SAMBA_LOGS_SCRIPT, {
...
describe("samba console interception", () => {
  ...
  it("preserves up to 50 object keys before marking additional keys", () => {
    ...
    expect(serialized.__samba_truncated__).toBe("Additional keys omitted");
  });
