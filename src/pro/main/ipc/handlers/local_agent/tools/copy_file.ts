...
const copyFileSchema = z.object({
  from: z
    .string()
    .describe(
      "The source file path (can be attachments:<name>, a .samba/media path, or a path relative to the app root)",
    ),
...
export const copyFileTool: ToolDefinition<z.infer<typeof copyFileSchema>> = {
  name: "copy_file",
  description:
    "Copy a file from one location to another. Can copy uploaded attachment files (attachments:<name> or .samba/media paths) into the codebase, or copy files within the codebase.",
  ...
  buildXml: (args, _isComplete) => {
    ...
    return `<samba-copy from="${escapeXmlAttr(args.from)}" to="${escapeXmlAttr(args.to)}" description="${escapeXmlAttr(args.description ?? "")}"></samba-copy>`;
  },
