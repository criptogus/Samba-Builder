...
import {
  buildSambaMediaUrlForApp,
  buildSambaMediaThumbnailUrl,
  buildSambaMediaUrl,
} from "../lib/sambaMediaUrl";
import {
...
import { createSambaMediaProtocolHandler } from "./samba_media_protocol";
...
describe("samba-media thumbnail protocol", () => {
  let root: string;
  let appPath: string;
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), "samba-media-protocol-"));
    appPath = path.join(root, "app");
    mediaPath = path.join(appPath, ".samba", "media");
  ...
  function makeHandler({
    ...
    const handler = createSambaMediaProtocolHandler({
      cacheRoot,
  ...
  it("resolves opaque app IDs without exposing the app path in the URL", async () => {
    ...
    const url = buildSambaMediaUrlForApp(7, "image.png");
  ...
  it("preserves legacy media URLs for an app folder named app-id", async () => {
    const legacyMediaPath = path.join(root, "app-id", ".samba", "media");
    ...
    await expect(
      handler(new Request(buildSambaMediaUrl("app-id", "legacy.png"))),
  ...
  it("serves originals only when a thumbnail was not requested", async () => {
    ...
    const result = await handler(
      new Request(buildSambaMediaUrl(appPath, "image.png")),
    );
  ...
  it("creates a bounded derivative once and reuses its mtime-keyed cache", async () => {
    ...
    const requestUrl = buildSambaMediaThumbnailUrl(
      appPath,
      "image.png",
      stat.mtimeMs,
      stat.size,
    ...
    expect(first.headers.get("x-samba-thumbnail-cache")).toBe("miss");
    expect(first.headers.get("cache-control")).toContain("immutable");
    expect(second.headers.get("x-samba-thumbnail-cache")).toBe("hit");
  ...
  it("reports coalesced cache misses accurately", async () => {
    ...
    expect(first.headers.get("x-samba-thumbnail-cache")).toBe("miss");
    expect(second.headers.get("x-samba-thumbnail-cache")).toBe("miss");
    expect(third.headers.get("x-samba-thumbnail-cache")).toBe("hit");
    expect(createThumbnailFromPath).toHaveBeenCalledTimes(1);
  });
  ...
  it("rejects oversized image dimensions before native decoding", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildSambaMediaUrl(appPath, "oversized.png")}?thumbnail=1`,
      ),
    );
  ...
  it("accepts JPEGs whose metadata places dimensions after 128 KiB", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildSambaMediaUrl(appPath, "metadata-heavy.jpg")}?thumbnail=1`,
      ),
    );
  ...
  it("rejects oversized source files before reading or decoding them", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildSambaMediaUrl(appPath, "too-large.png")}?thumbnail=1`,
      ),
    );
  ...
  it("reports empty image files separately from oversized files", async () => {
    ...
    const result = await handler(
      new Request(`${buildSambaMediaUrl(appPath, "empty.png")}?thumbnail=1`),
    );
  ...
  it("falls back safely for corrupt images and oversized output", async () => {
    ...
    const corruptResult = await corrupt.handler(
      new Request(`${buildSambaMediaUrl(appPath, "corrupt.png")}?thumbnail=1`),
    );
    ...
    const outputResult = await oversizedOutput.handler(
      new Request(
        `${buildSambaMediaUrl(appPath, "large-output.png")}?thumbnail=1`,
      ),
    );
    ...
    const invalidOutputResult = await invalidOutput.handler(
      new Request(
        `${buildSambaMediaUrl(appPath, "invalid-output.png")}?thumbnail=1`,
      ),
    );
  ...
  it("decodes a bounded snapshot when the source changes during generation", async () => {
    ...
    const result = await handler(
      new Request(`${buildSambaMediaUrl(appPath, "image.png")}?thumbnail=1`),
    );
  ...
  it("rejects symlinks that escape the media directory", async () => {
    ...
    const result = await handler(
      new Request(`${buildSambaMediaUrl(appPath, "escape.png")}?thumbnail=1`),
    );
