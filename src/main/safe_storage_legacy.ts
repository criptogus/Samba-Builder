 ...
 * the "samba Safe Storage" item (used post-`ready` / Electron 43). The
...
// Ordered by likelihood on a current install: the post-`ready` "samba" identity
...
let inProcessBindingLoader: KeychainReaderBindingLoader = () =>
  require("samba-keychain-reader") as KeychainReaderBinding;
...
function createDefaultKeychainPasswordReader(): KeychainPasswordReader {
  return process.env.SAMBA_SAFE_STORAGE_READER === "cli"
...
export function recoveryNeedsKeychainUnlock(): boolean {
  ...
  if (process.env.SAMBA_DISABLE_SAFE_STORAGE_RECOVERY === "1") {
    return false;
  }
  if (process.env.SAMBA_DISABLE_SAFE_STORAGE_UNLOCK_PROMPT === "1") {
    return false;
  }
 ...
 * non-darwin, the `SAMBA_DISABLE_SAFE_STORAGE_RECOVERY=1` kill switch, or a
