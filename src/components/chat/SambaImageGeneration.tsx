...
interface SambaImageGenerationNode {
  properties: {
    prompt: string;
    path: string;
...
interface SambaImageGenerationProps {
  children?: ReactNode;
  node?: SambaImageGenerationNode;
}

export const SambaImageGeneration: React.FC<SambaImageGenerationProps> = ({
  children,
  node,
}) => {
  ...
  const imageUrl =
    appPath && normalizedImagePath && !hasTraversal
      ? `samba-media://media/${encodeURIComponent(appPath)}/${normalizedImagePath
          .split("/")
        ...
        <div className="flex items-start">
          <div className="flex-1 min-w-0">
            <SambaCardHeader
              ...
              accentColor="violet"
            >
              <SambaBadge color="violet">Image Generation</SambaBadge>
              {!isExpanded && prompt && (
