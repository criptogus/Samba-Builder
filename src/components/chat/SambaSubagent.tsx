...
import {
  SambaCard,
  SambaCardHeader,
  SambaCardPresentationContext,
} from "./SambaCardPrimitives";

interface SambaSubagentProps {
  chatId: number;
  threadId: string;
  persona: string;
  specialist?: string;
      ...
      {isExpanded && (
        <div className="border-t border-border/60 px-3 py-2">
          {loadError ? (
            ...
            <SambaCardPresentationContext.Provider value="embedded">
            ...
            </SambaCardPresentationContext.Provider>
          ) : isActive ? (
            <p className="py-2 text-xs text-muted-foreground">
...
function withActivityState(
  ...
  return xml.replace(/^(<samba-[\w-]+)/, `$1 state="${state}"`);
}
