...
function makeActivity(): SubagentActivity {
  return {
    ...
    status: "completed",
    presentationXml:
      '<samba-grep query="auth" count="1">src/auth.ts:1: auth</samba-grep>',
    error: null,
    startedAt: new Date(),
...
describe("SambaSubagent", () => {
  ...
  it("shows live tool activity and collapses into findings when complete", async () => {
    ...
    expect(await screen.findByText(/samba-grep state/)).toBeTruthy();
    ...
    await waitFor(() => {
      expect(screen.queryByText(/samba-grep state/)).toBeNull();
      expect(
