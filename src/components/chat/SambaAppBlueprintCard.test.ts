...
const templates: Template[] = [
  ...
  {
    id: "samba-sh/community-template",
  ...
  {
    id: "samba-sh/selected-community-template",
  ...
  it("keeps the current community template visible", () => {
    expect(
      getAppBlueprintTemplateOptions(
        templates,
        "samba-sh/selected-community-template",
      ).map((template) => template.id),
    ).toEqual(["react", "samba-sh/selected-community-template"]);
  });
});
