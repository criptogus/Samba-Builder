...
describe("SambaMarkdownParser samba-status", () => {
  afterEach(() => {
  ...
  it("honors explicit aborted state on closed status tags", () => {
    render(
      <SambaMarkdownParser
        content={
          '<samba-status title="Supabase functions failed" state="aborted">\n0 succeeded\n1 failed\n</samba-status>'
        }
      />,
    );
  ...
  it("renders incomplete type checks as amber warnings", () => {
    const { container } = render(
      <SambaMarkdownParser
        content={
          '<samba-status title="Type check incomplete" state="warning">Fix the configuration error, then rerun type checking.</samba-status>'
        }
      />,
    );
  ...
  it("keeps completed checks with source errors in the green finished state", () => {
    const { container } = render(
      <SambaMarkdownParser
        content={
          '<samba-status title="Type errors found" state="finished">Found 1 type error.</samba-status>'
        }
      />,
    );
...
describe("SambaMarkdownParser samba-command", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders the add TypeScript command as a supported action button", () => {
    render(
      <SambaMarkdownParser
        content={'<samba-command type="add-typescript"></samba-command>'}
      />,
    );
...
describe("SambaMarkdownParser samba-explore-chat-history", () => {
  ...
  it.each(["partial", "no_match"])(
    ...
    (outcome) => {
      render(
        <SambaMarkdownParser
          content={`<samba-explore-chat-history query="auth" chats="1" evidence="2" outcome="${outcome}">Historical evidence</samba-explore-chat-history>`}
        />,
      );

      expect(
        screen
          .getByTestId("samba-explore-chat-history")
...
describe("SambaMarkdownParser samba-git", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it.each([
    ...
    render(
      <SambaMarkdownParser
        content={`<samba-git operation="${operation}" revision="abc123" path="src/main.ts"></samba-git>`}
      />,
    );
  ...
  it("renders a pending state while the Git tag streams", () => {
    ...
    render(
      <Provider store={store}>
        <SambaMarkdownParser
          content={'<samba-git operation="status" state="pending">'}
        />
      </Provider>,
    );
  ...
  it("shows a useful status summary and expands structured file details", () => {
    ...
    render(
      <SambaMarkdownParser
        content={`<samba-git operation="status" branch="main" changed_count="2" untracked_count="1" conflicted_count="0" detail_format="status">${status}</samba-git>`}
      />,
    );
  ...
  it("does not render malformed status detail as structured content", () => {
    render(
      <SambaMarkdownParser
        content={
          '<samba-git operation="status" detail_format="status">{"branch":"main","detached":false,"staged":[{}],"unstaged":[],"untracked":[],"conflicted":[]}</samba-git>'
        }
      />,
    );
...
describe("SambaMarkdownParser samba-security-finding", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders a card with the title and severity level", () => {
    render(
      <SambaMarkdownParser
        content={
          '<samba-security-finding title="SQL Injection in User Lookup" level="critical">\nUser input is concatenated into a query.\n</samba-security-finding>'
        }
      />,
    );
  ...
  it("falls back gracefully when the level is missing or invalid", () => {
    render(
      <SambaMarkdownParser
        content={'<samba-security-finding title="Partial finding">'}
      />,
    );
...
describe("SambaMarkdownParser closed-block render counts", () => {
  beforeEach(() => {
  ...
  // Three markdown segments separated by two closed samba-status tags.
  ...
  const TAG1 = '<samba-status title="S1" state="finished">ok</samba-status>';
  const MD2 = "\n\nSecond paragraph content.\n\n";
  const TAG2 = '<samba-status title="S2" state="finished">ok</samba-status>';
  const MD3 = "\n\nThird paragraph content.";
  const FULL = MD1 + TAG1 + MD2 + TAG2 + MD3;

  it("renders each markdown block exactly once for a one-shot parse", () => {
    render(<SambaMarkdownParser content={FULL} />);
  ...
  it("does not re-render closed markdown blocks as later content streams in", () => {
    ...
    const { rerender } = render(
      <SambaMarkdownParser content={FULL.slice(0, md1ClosesAt)} />,
    );
    ...
    for (let i = md1ClosesAt + 1; i <= md2ClosesAt; i++) {
      rerender(<SambaMarkdownParser content={FULL.slice(0, i)} />);
    }
...
describe("SambaMarkdownParser parser-cache perf metrics", () => {
  ...
  function streamFully() {
    const { rerender } = render(
      <SambaMarkdownParser content={FULL.slice(0, 1)} />,
    );
