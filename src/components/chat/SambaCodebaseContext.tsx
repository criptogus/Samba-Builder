...
interface SambaCodebaseContextProps {
...
export const SambaCodebaseContext: React.FC<SambaCodebaseContextProps> = ({
  node,
}) => {
  ...
  return (
    ...
    >
      <SambaCardHeader icon={<Code2 size={15} />} accentColor="blue">
        <SambaBadge color="blue">Codebase Context</SambaBadge>
        {files.length > 0 && (
          <span className="text-xs text-muted-foreground">
            Using {files.length} file{files.length !== 1 ? "s" : ""}
          </span>
        )}
        {inProgress && <SambaStateIndicator state="pending" />}
