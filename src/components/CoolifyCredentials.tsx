...
/**
 * The way into a server Samba Builder set up.
 *
 * Samba Builder invents the admin password and mints the API token, so it is the only
 ...
 * making them click to discover that Samba Builder even has them means most people
 ...
 * the absence is Samba Builder holding one it cannot open — not a server that never
...
function LockedPassword() {
  return (
    ...
    >
      Samba Builder is holding an admin password for this server but cannot read
      it on this machine.
    </p>
  );
}
...
export function CoolifyCredentials({
  ...
  // blank where those belong reads as Samba Builder holding nothing rather than as a
  ...
  if (isPending && !isError) {
    return (
      ...
      >
        Looking up what Samba Builder has stored…
      </p>
    );
  }
  ...
  // retry — still leaves them readable, and taking a password Samba Builder holds the
  ...
  if (isError && !credentials) {
    return (
      ...
      >
        Samba Builder could not read what it has stored for this Coolify.
      </p>
    );
  }
  ...
  // The usual case: Samba Builder set the server up and is connected to it. Merged on
  ...
  return (
    <div className="space-y-2 text-sm" data-testid="coolify-credentials">
      ...
      {/* A password Samba Builder holds but cannot decrypt comes back absent, which
        ...
        <>
            ...
            <div className="space-y-2" data-testid="coolify-credentials-server">
              {showsBoth && (
                <div className="text-muted-foreground text-xs">
                  The server Samba Builder set up
                </div>
              )}
              <Field
                label="Address"
          ...
          {instance && (
            ...
            >
              {showsBoth && (
                <div className="text-muted-foreground text-xs">
                  The Coolify Samba Builder is connected to
                </div>
              )}
              <Field
                label="Address"
