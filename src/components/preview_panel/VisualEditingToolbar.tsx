...
export function VisualEditingToolbar({
  ...
  const handleDeselectComponent = () => {
    ...
    if (iframeRef.current?.contentWindow) {
      iframeRef.current.contentWindow.postMessage(
        {
          type: "remove-samba-component-overlay",
  ...
  const sendStyleModification = (styles: {
    ...
    iframeRef.current.contentWindow.postMessage(
      {
        type: "modify-samba-component-styles",
        data: {
    ...
    iframeRef.current.contentWindow.postMessage(
      {
        type: "update-samba-overlay-positions",
      },
      "*",
    );
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === "samba-component-styles") {
