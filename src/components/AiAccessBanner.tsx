...
import { hasSambaProKey } from "@/lib/schemas";
...
export function ProBanner() {
  ...
  if (settings && hasSambaProKey(settings)) {
    return null;
  }
...
export function ManageSambaProButton({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {t("proBanner.manageSambaPro")}
      <ArrowUpRight aria-hidden="true" className="w-5 h-5" />
    </Button>
  );
}

export function SetupSambaProButton() {
  const { t } = useTranslation("home");
  return (
    <button
      type="button"
    ...
    >
      ...
      <div className="relative z-10 text-center flex flex-col items-center gap-0.5 sm:gap-1 md:gap-1.5 px-4 md:px-6 pr-6 md:pr-8">
        <div className="mt-0.5 sm:mt-1 flex items-center gap-2 sm:gap-3 justify-center">
          ...
          <button
            type="button"
            aria-label="Subscribe to Samba Builder"
