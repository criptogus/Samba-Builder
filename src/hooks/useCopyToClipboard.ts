...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as SambaMarkdownParser but convert to clean text
      const formattedContent = convertSambaContentToMarkdown(messageContent);
  ...
  // Convert Samba Builder content to clean markdown using the same parsing logic as SambaMarkdownParser
  const convertSambaContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from SambaMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from SambaMarkdownParser)
  ...
  // Reuse the same parsing functions from SambaMarkdownParser but simplified
