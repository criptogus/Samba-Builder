...
export function registerTokenCountHandlers() {
  handle(
    ...
    async (event, req: TokenCountParams): Promise<TokenCountResult> => {
      ...
      const frameworkType = detectFrameworkType(getSambaAppPath(chat.app.path));
      ...
      let systemPrompt = constructSystemPrompt({
        aiRules: await readAiRules(getSambaAppPath(chat.app.path)),
      ...
      const isSambaPro = isSambaProEnabled(settings);
      const mcpToolDefs =
