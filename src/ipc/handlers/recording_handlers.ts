 ...
 * preview goes on acting as a user Samba Builder has disowned, against the real project.
 *
    ...
    async (event, params): Promise<StartRecordingResult> => {
      ...
      if (!startReservation) {
        ...
        return infraResult(
          appId,
          blockedBy
            ? `Wait for Samba Builder to ${blockedBy} before starting a recording.`
      ...
      try {
        ...
        if (app.neonTestBranchId) {
          ...
          if (!restored) {
            return infraResult(
              appId,
              "Samba Builder couldn't restore this app's real database settings from the previous session. Retry after checking the Neon connection.",
            );
          }
              ...
              try {
                ...
                if (
                  ...
                  ready.resolve(
                    infraResult(
                      appId,
                      "Samba Builder couldn't restore this app's real database settings from the previous operation. Retry after checking the Neon connection.",
                    ),
                  );
                  return;
                }
                ...
                if (!summary.envRestored) {
                  ...
                  endReason = "error";
                  endMessage =
                    "Samba Builder couldn't restore your app's real database settings after recording. Restore .env.local before running the app again.";
                }
                clearRegistration();
