  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getSambaChatSummaryTag(messageContent);

        const proposalWriteFiles = getSambaWriteTags(messageContent);
        const proposalSearchReplaceFiles =
          getSambaSearchReplaceTags(messageContent);
        const proposalRenameFiles = getSambaRenameTags(messageContent);
        const proposalDeleteFiles = getSambaDeleteTags(messageContent);
        const proposalExecuteSqlQueries =
          getSambaExecuteSqlTags(messageContent);
        const packagesAdded = getSambaAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getSambaWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getSambaCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getSambaChatSummaryTag(messageToApprove.content);
