...
vi.mock("electron", () => ({
  ...
  app: {
    getPath: vi.fn(() => path.join(os.tmpdir(), "samba-media-user-data")),
...
describe("media move recording admission", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "samba-media-handler-"));
  ...
  it("allows a move out of a recording source when the target is free", async () => {
    ...
    const sourceMediaPath = path.join(sourcePath, SAMBA_MEDIA_DIR_NAME);
    ...
    expect(
      fs.existsSync(path.join(targetPath, SAMBA_MEDIA_DIR_NAME, "image.png")),
    ).toBe(true);
  });
});
