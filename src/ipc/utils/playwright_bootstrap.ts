  ...
  if (has("pnpm-lock.yaml")) {
    return {
      ...
      args: [
        ...PNPM_INSTALL_POLICY_ARGS,
        "add",
        // Samba Builder can generate a pnpm-workspace.yaml (for install policy), which
...
export const TEST_BASE_URL_ENV = "SAMBA_TEST_BASE_URL";
 ...
 */
export const TEST_SLOW_MO_ENV = "SAMBA_TEST_SLOW_MO";
 ...
 * written one.
 */
const SLOW_MO_CONFIG_LINES = `    // Slow motion: Samba Builder sets ${TEST_SLOW_MO_ENV} (milliseconds between
...
/**
 * Samba Builder's own Playwright config. Deliberately NOT `playwright.config.ts`: that
 ...
 * name and is selected explicitly with `--config` on every run, so Samba Builder never
 * has to rename, back up, or overwrite a config it doesn't own.
 */
export const SAMBA_CONFIG_FILENAME = "playwright-samba.config.ts";
...
/**
 * Samba Builder-generated preview plumbing, regenerated on every run. Committing it
 * would hand a teammate who doesn't use Samba Builder a tsconfig that shadows the app's
 ...
 * and warns instead — so an unconditional rule would gitignore a file Samba Builder
 ...
 * for the files Samba Builder actually generated.
 */
const PREVIEW_GITIGNORE_ENTRIES = {
  tsconfig: "/e2e-tests/tsconfig.json",
  shim: "/e2e-tests/fixtures/samba/",
} as const;
 ...
 */
const SAMBA_CONFIG_SENTINEL = "Generated by Samba Builder";

/**
 * Environment variable carrying Samba Builder's CDP endpoint. Only set for preview
 * runs; the generated fixture shim stays inert without it, so every other run
 * launches its own browser exactly as before.
 */
export const PREVIEW_CDP_ENDPOINT_ENV = "SAMBA_PREVIEW_CDP_ENDPOINT";

/** Bearer token for the run-scoped, one-preview CDP broker. */
export const PREVIEW_CDP_TOKEN_ENV = "SAMBA_PREVIEW_CDP_TOKEN";
 ...
 */
const PREVIEW_SHIM_DIR = `${E2E_TEST_DIR}/fixtures/samba`;

/**
 * Fixture shim that reroutes the default `page` onto the page already loaded in
 * Samba Builder's preview panel. Specs reach it through the tsconfig path mapping below
 * rather than by importing it, so existing specs need no edits.
 */
export const PREVIEW_SHIM_RELATIVE_PATH = `${PREVIEW_SHIM_DIR}/samba-test.ts`;

/**
 * Where the shim lived before it moved into its own directory. Apps bootstrapped
 * by an older Samba Builder still have this file; it is dead once the mapping below
 * points at the new location, so we clean it up.
 */
const LEGACY_PREVIEW_SHIM_RELATIVE_PATH = `${E2E_TEST_DIR}/fixtures/samba-test.ts`;
...
/** Where an older Samba Builder's mapping pointed, relative to the same tsconfig. */
 ...
 * extension are optional — reading "./fixtures/samba-test" as "not the legacy
...
/**
 * Builds the `playwright-samba.config.ts` source. When `channel` is set, the
...
// ${SAMBA_CONFIG_SENTINEL}. The dev server is started separately by Samba Builder's
...
export function buildPreviewShimSource(): string {
  return `// ${SAMBA_CONFIG_SENTINEL}. Do not edit — Samba Builder regenerates this file.
//
// Lets a headed run drive the page already open in Samba Builder's preview panel
// instead of launching a separate browser. Inert unless Samba Builder sets
...
function requireBaseUrl(): string {
  ...
  if (!baseUrl) {
    throw new Error("Samba Builder preview: ${TEST_BASE_URL_ENV} is not set.");
  }
  return baseUrl;
}
...
function requirePreviewContext(browser: pw.Browser) {
  ...
  if (contexts.length !== 1) {
    throw new Error(
      \`Samba Builder preview: the automation broker exposed \${contexts.length} browser contexts instead of exactly one.\`,
    );
  }
  return contexts[0];
}
...
async function ensurePreviewBetterAuthSession({
  ...
  if (!result.ok) {
    throw new Error(
      \`Samba Builder preview: Better Auth sign-in returned \${result.status}, but no browser session was established.\`,
    );
  }
}
...
export const test = !endpoint
  ...
  : pw.test.extend({
      browser: [
        async ({}, use) => {
          if (!cdpToken) {
            throw new Error("Samba Builder preview: ${PREVIEW_CDP_TOKEN_ENV} is not set.");
          }
          ...
          try {
            ...
            // Disconnects this client; it never closes Samba Builder itself.
            await browser.close();
          }
        },
        { scope: "worker" },
      ...
      context: async ({ browser }, use) => {
        ...
        // isolated preview context and nothing from Samba Builder's privileged session.
        const context = requirePreviewContext(browser);
        // Playwright applies the config's \`baseURL\` when IT creates a context.
        // Samba Builder's preview created this one, so the option is missing and every
        ...
        // so a spec behaves the same whichever way Samba Builder runs it.
      ...
      page: async ({ context }, use, testInfo) => {
        ...
        if (!page) {
          throw new Error(
            \`Samba Builder preview: no page serving \${origin} — is the preview panel showing this app?\`,
          );
        }
        ...
        try {
          // Pre-existing page: leave it open for Samba Builder to replace after this
          ...
          // Samba Builder picks up even in an app whose config predates that change.
...
export function buildE2eTsconfigSource(appPath: string): string {
  ...
  return `${JSON.stringify(
    {
      _comment: `${SAMBA_CONFIG_SENTINEL}. Routes @playwright/test through Samba Builder's fixture shim so tests can run inside the preview panel. The other entries keep this app's own tsconfig settings, which this file would otherwise shadow. Delete this file to opt out.`,
...
export function buildShimTsconfigSource(): string {
  return `${JSON.stringify(
    {
      _comment: `${SAMBA_CONFIG_SENTINEL}. Turns off the @playwright/test path mapping from ../../tsconfig.json so the shim beside this file can import the real package. Do not add sources to this directory.`,
      compilerOptions: {
        paths: {},
      },
    },
 ...
 */
/**
 * Rewrites the Samba Builder-generated `e2e-tests/tsconfig.json` from the app's current
...
export function refreshGeneratedE2eTsconfig(appPath: string): void {
  ...
  if (existing === null || !existing.includes(SAMBA_CONFIG_SENTINEL)) return;
...
export function ensurePreviewShim(appPath: string): { warning?: string } {
  ...
  if (existingShim === null || existingShim.includes(SAMBA_CONFIG_SENTINEL)) {
  ...
  if (
    ...
    existingShimTsconfig.includes(SAMBA_CONFIG_SENTINEL)
  ) {
  ...
  const appOwnedTsconfig =
    ...
    !existingTsconfig.includes(SAMBA_CONFIG_SENTINEL)
      ? existingTsconfig
      : null;

  // An older Samba Builder kept the shim one directory up and told users to map to it
  ...
  // an earlier Samba Builder already deleted the file out from under the mapping).
  ...
  if (rootRoutesToShim) {
    ...
    if (closerTsconfig) {
      return {
        warning: `${closerTsconfig} takes precedence for at least one test and doesn't route "@playwright/test" through Samba Builder's preview shim. The run will use a separate browser instead. Extend ${E2E_TSCONFIG_RELATIVE_PATH} or add a mapping to the generated shim to enable preview runs.\n`,
      };
    }
    // The app kept its own root tsconfig, or every closer config, routing to a
    // shim Samba Builder maintains.
    return {};
  }

  return {
    warning: `Your app has its own ${E2E_TSCONFIG_RELATIVE_PATH}, so Samba Builder can't route tests through the preview. The run will use a separate browser instead. Add a "@playwright/test" path mapping to "${SHIM_PATH_FROM_E2E_TSCONFIG}" to enable preview runs.\n`,
  };
}
...
/** Absolute path of Samba Builder's own config. The user's config is never touched. */
function sambaConfigPath(appPath: string): string {
  return path.join(appPath, SAMBA_CONFIG_FILENAME);
}

function hasSambaPlaywrightConfig(appPath: string): boolean {
  return fs.existsSync(sambaConfigPath(appPath));
}

/** Read Samba Builder's config source, or null if we haven't written one yet. */
function readSambaConfigText(appPath: string): string | null {
  const file = sambaConfigPath(appPath);
...
/** True when Samba Builder's config drives a system browser via `channel`. */
function configUsesChannel(appPath: string): boolean {
  const text = readSambaConfigText(appPath);
  return (
    text != null &&
...
/**
 * True when Samba Builder's config names a `timeout` of its own. The template never
...
/** True when Samba Builder's config is still pure template output (safe to rewrite). */
function isSambaGeneratedConfig(appPath: string): boolean {
  const text = readSambaConfigText(appPath);
  return text != null && text.includes(SAMBA_CONFIG_SENTINEL);
}
...
function writePlaywrightConfig(
  ...
  fs.writeFileSync(sambaConfigPath(appPath), buildPlaywrightConfig(channel));
  logger.info(
    `Wrote ${SAMBA_CONFIG_FILENAME} (${channel ? `channel: ${channel}` : "bundled chromium"})`,
  );
}

/**
 * Rewrite a Samba Builder-generated config that still points `testDir` at the legacy
...
function migrateConfigTestDir(appPath: string): void {
  if (!isSambaGeneratedConfig(appPath)) return;
  const text = readSambaConfigText(appPath);
  const legacy = 'testDir: "./tests"';
  const current = `testDir: "./${E2E_TEST_DIR}"`;
  if (text && text.includes(legacy)) {
    fs.writeFileSync(sambaConfigPath(appPath), text.replace(legacy, current));
    logger.info(
      `Migrated ${SAMBA_CONFIG_FILENAME} testDir to ./${E2E_TEST_DIR}`,
    );
  }
}

/**
 * Add the slow-motion `launchOptions` to a Samba Builder-generated config written before
 * the Tests panel had that toggle — without it the toggle would silently do
 * nothing for apps bootstrapped by an older Samba Builder. Splices in just those lines
...
function migrateConfigSlowMo(appPath: string): void {
  ...
  fs.writeFileSync(
    sambaConfigPath(appPath),
    text.replace(baseUrlLine, `${baseUrlLine}\n${SLOW_MO_CONFIG_LINES}`),
  );
  logger.info(`Added slow-motion support to ${SAMBA_CONFIG_FILENAME}`);
}

/**
 * Turns Playwright's own screenshot/trace recorders off for preview runs in a
 * Samba Builder-generated config written before preview runs existed.
 *
 ...
 * a failing test in an app bootstrapped by an older Samba Builder still captures every
 * page reachable over the borrowed CDP connection, which includes Samba Builder's own
...
function migrateConfigPreviewRecorders(appPath: string): void {
  ...
  logger.info(
    `Scoped screenshot/trace capture to non-preview runs in ${SAMBA_CONFIG_FILENAME}`,
  );
}
...
function ensureTestScript(appPath: string): void {
  ...
  try {
    ...
    if (!pkg.scripts.test) {
      // Point at Samba Builder's config explicitly — Playwright only auto-resolves
      // `playwright.config.ts`, which is the app's own file, not ours.
      pkg.scripts.test = `playwright test --config ${SAMBA_CONFIG_FILENAME}`;
      ...
      // Migrate the old Samba Builder-generated bare script, but only when the app has
      ...
      // global setup, and break `npm test` outside Samba Builder. With no config of
      // their own, the bare script can only be Samba Builder's older template output.
      // (Scripts with flags, env vars, or extra commands are never touched.)
      pkg.scripts.test = `playwright test --config ${SAMBA_CONFIG_FILENAME}`;
      fs.writeFileSync(packageJsonPath, `${JSON.stringify(pkg, null, 2)}\n`);
      logger.info("Updated Samba Builder test script to use explicit config");
    }
  } catch (err) {
 ...
 *
 * Throws a SambaError (kind External) on install failure — typically offline,
...
export async function ensurePlaywrightBootstrap({
  ...
  if (!packageInstalled) {
    ...
    const installDep = await spawnStreaming({
      ...
      // Samba Builder-managed package-manager paths (executeAddDependency, runtime).
  ...
  // Samba Builder's config lives under its own name and every run selects it with
  ...
  if (!hasSambaPlaywrightConfig(appPath)) {
    ...
    if (!usesChannel && detectedChannel && isSambaGeneratedConfig(appPath)) {
  ...
  // Bring an older Samba Builder config's testDir up to date (./tests -> ./e2e-tests) so
  ...
  // there). Only needed when we're NOT driving a system browser or Samba Builder's
