...
/**
 * Copy a file within a Samba Builder app, with security validation, git staging,
 * and optional Supabase function deployment.
 *
 * @throws Error if an absolute source path is outside the app's .samba/media directory.
...
export async function executeCopyFile({
  ...
  const readsMedia =
    ...
    normalizedSource === SAMBA_MEDIA_DIR_NAME ||
    normalizedSource.startsWith(`${SAMBA_MEDIA_DIR_NAME}/`);
  return appOperationCoordinator.run(
    ...
    async () => {
      ...
      // Resolve the source path: allow both .samba/media paths and app-relative paths
      ...
      if (from.startsWith("attachments:")) {
        ...
        // Security: only allow absolute paths within the app's .samba/media directory
        if (!isWithinSambaMediaDir(from, appPath)) {
          throw new Error(
            `Absolute source paths are only allowed within the .samba/media directory`,
          );
        }
      ...
      if (
        path.isAbsolute(from) &&
        !isWithinSambaMediaDir(realFromPath, resolvedAppPath)
      ) {
        throw new Error(
          `Source path resolves to a location outside the .samba/media directory (possible symlink traversal)`,
        );
      }
      if (
        !path.isAbsolute(from) &&
