...
describe("buildPreviewShimSource", () => {
  ...
  it("stays inert unless Samba Builder hands it an endpoint", () => {
    expect(source).toContain(`process.env.${PREVIEW_CDP_ENDPOINT_ENV}`);
    expect(source).toContain("!endpoint\n  ? pw.test");
  });

  it("attaches a screenshot of the page under test, not of Samba Builder", () => {
    // Playwright's own recorder shoots every page the connection can reach,
    // and over CDP that includes Samba Builder's windows — so a failing preview test
...
describe("ensurePreviewShim", () => {
  ...
  it("removes the shim an older Samba Builder left at the fixtures root", () => {
    const appPath = makeApp();
    ...
    fs.writeFileSync(
      legacyShimPath,
      "// Generated by Samba Builder. old shim\n",
    );
  ...
  it("keeps the old shim alive while the app's tsconfig still maps to it", () => {
    ...
    // The mapping an older Samba Builder's warning told the user to add. Deleting the
  ...
  it("restores an old shim a previous Samba Builder deleted out from under the mapping", () => {
    const appPath = makeApp();
  ...
  it("refreshes its own files without asking", () => {
    ...
    fs.writeFileSync(
      shimAt(appPath),
      "// Generated by Samba Builder. stale contents\n",
    );
...
describe("ensurePlaywrightBootstrap", () => {
  ...
  it("writes its own config and never touches the app's playwright.config.ts", async () => {
    ...
    // Samba Builder no longer takes over the canonical config name.
    expect(fs.readFileSync(userConfigPath, "utf8")).toBe(userConfig);
    expect(fs.existsSync(`${userConfigPath}.backup`)).toBe(false);
  });

  it("migrates an older Samba Builder-generated config's testDir to ./e2e-tests", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("scopes an older Samba Builder-generated config's recorders away from preview runs", async () => {
    ...
    // by an older Samba Builder still captures every page reachable over the borrowed
    // CDP connection — which includes Samba Builder's own product windows.
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("teaches an older Samba Builder-generated config the slow-motion option", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
    ...
    // bootstrapped by an older Samba Builder.
    expect(updated).toContain(
      `launchOptions: { slowMo: Number(process.env.${TEST_SLOW_MO_ENV}) || 0 }`,
    );
  ...
  it("leaves a config without the Samba Builder sentinel untouched", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("is a no-op when a Samba Builder config already targets ./e2e-tests", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("points the package.json test script at Samba Builder's config", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("migrates the old Samba Builder-generated bare test script", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("leaves a bare test script alone when the app owns a playwright.config", async () => {
    ...
    // and global setup, and break `npm test` outside Samba Builder.
    fs.writeFileSync(
