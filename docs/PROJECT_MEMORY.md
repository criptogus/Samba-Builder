...
**Regressão do `@lexical/utils` (2026-09-17):** o build do renderer quebrou de novo com "Could not resolve @lexical/utils" porque a declaração **desapareceu do `package.json` e do lockfile** — não era `node_modules` incompleto, era a dependência perdida numa reconciliação (o `lexical-beautiful-mentions` importa `@lexical/utils` como peer, e sem entrada no topo o bundler não resolve). Restaurado em `dependencies` (`^0.33.1`, mesma linha do `lexical` e `@lexical/react`) e materializado pelo `npm install` do start do app. Trava nova: `src/__tests__/lexical_deps.test.ts` exige a declaração, exige o trio na mesma versão de monorepo e exige a entrada no lockfile — antes disso, a perda só aparecia como app que não abre.
...
- **Pull request automático (REQ-32):** opção `autoOpenPullRequest` (desligada por padrão; ver as 6 etapas da regra `rules/adding-settings.md`: schema, `DEFAULT_SETTINGS`, search index, switch, página e snapshots de `src/main/settings.test.ts`). Gatilho: **fim de um push bem-sucedido** — `github_ops_service.ts` (caso `push`, logo após `handlePushToGithub`) chama `ensurePullRequestAfterPush` de `github_handlers.ts`, com a decisão pura em `src/ipc/services/github/auto_pull_request.ts`. Nunca abre PR na branch padrão, não duplica PR aberto e roda em **melhor esforço**: falha ao abrir o PR **não** derruba o push. O renderer sabe pelo evento `git:pull-request-opened` (exposto de graça pelo preload, porque `gitEvents` já entra em `getReceiveChannels`) e mostra o link em `GithubPullRequestActions`.
- **Sync (REQ-33):** a operação `sync` vive na máquina (`src/github_ops/state.ts`) e é um **composto**: `compositeNext(sync)` devolve `push`, e `github_ops_service.ts` executa o pull para `sync`. A ordem importa — a máquina descarta pedidos novos enquanto roda, então dois cliques (pull, push) perderiam o segundo. Quem for adicionar operação nova precisa tocar em: `state.ts` (união), `transport.ts` (schema do wire **e** `operationTypeSchema`), `transition.ts` (`compositeNext`, `completionCommands`, `successBannerContent`, `continuationOperation`), `projection.ts` (`isSyncing`), o serviço (`getGithubRecordingRefusal` + `runUnlocked`) e `REPRESENTATIVE_OPS` em `transition.test.ts` (matriz de totalidade).
- **Sync em duas superfícies (REQ-33):** o botão "Sync to GitHub" do `GitHubConnector` e o item "Sync with GitHub" (`data-testid="git-sync-button"`) no menu de ações do `GithubBranchManager` despacham a mesma operação `{ type: "sync" }` e usam `isSyncing` para o spinner. A UI não monta o fluxo: quem garante a ordem (pull antes do push) é o composto da máquina, então uma segunda superfície não pode divergir.
- **O PR automático vale para push, sync e rebase (REQ-32 + REQ-33):** o hook fica **só** no caso `push` de `github_ops_service.ts`; como o composto da máquina despacha um `run-op push` real ao fim de um sync (`compositeNext`) e de um rebase, esses fluxos herdam o PR automático sem um segundo hook que pudesse divergir. Garantia em teste: rodar `sync` **não** abre PR (baixar não é motivo) e o push encadeado abre exatamente um.
- **Conflito no pull de um sync (REQ-33):** teste de integração no harness do ator (`src/github_ops/main_actor.test.ts`, "mantém o sync pendente quando o pull acha conflito…") dirige definição e transporte **reais** com o serviço de git falso: o pull falha com `code: "MERGE_CONFLICT"` → a máquina fica `conflicted` com `origin: sync` e **nada é enviado**; depois da resolução (`RESOLVE_WITH_AI_STARTED` → `CONFLICT_RESOLUTION_STARTED` → `CONFLICT_RESOLUTION_FINISHED` → conflitos vazios → `ready-to-sync`), continuar refaz o sync inteiro. A sequência observada de execuções é `["sync", "sync", "push"]`; se `continuationOperation` voltasse a devolver `push` para o sync, a continuação seria ignorada (`blocked-by-conflicts`) e a sequência pararia em `["sync"]` — ou seja, o teste falha se a regra regredir.
- **Não existe:** push automático (o agente nunca dá push sozinho) e merge automático de PR — as duas seriam decisões de produto separadas.
