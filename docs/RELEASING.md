...
3. Dispare o workflow **Release app** (`workflow_dispatch`), ou empurre a tag `v<versão>` para publicar.
4. O workflow cria a tag a partir da versão (no caminho da tag, apenas **valida** a tag empurrada), builda **4 alvos** em paralelo (macOS, macOS Intel, Windows, Linux) e monta a release como **rascunho**.
5. Ainda no workflow: confere os assets contra a proveniência dos builds e, **no caminho da tag**, publica a release. O disparo manual termina em rascunho, para revisar antes — publicar é empurrar a tag.
...
```sh
gh workflow run release.yml -R criptogus/Samba-Builder --ref main   # builda e termina em rascunho, para revisar
git push origin v<versão>                                           # caminho da tag: valida e publica
...
- **Prepare Release Tag** — no disparo manual, `node scripts/prepare-release-tag.js prepare`: lê a versão do `package.json`, cria (ou reusa) a tag `v<versão>` e a release **não publicada**. No caminho da tag ela já existe: o passo só confere que ela bate com a versão do `package.json`.
...
- **Publish Release** — baixa os artefatos, `electron-forge publish --from-dry-run` (cria o **rascunho**), anexa os manifestos, revalida a tag, checa os assets com `scripts/verify-release-assets.js` e, **no caminho da tag**, publica (`gh release edit --draft=false`, com `--prerelease` para versão com sufixo).
...
As notas da release vêm de `docs/releases/v<versão>.md` (versionada, revisável no pull request). Sem o arquivo, o corpo é montado com a lista de commits entre a release anterior e esta — assim o "novidades desta versão" do app não abre em branco.
...
No **caminho da tag** o workflow publica sozinho depois da verificação; o disparo manual termina em rascunho — e **rascunho é invisível para a checagem de versão do app**, que responde "você está atualizado" enquanto isso. Foi essa a confusão que originou esta seção.
