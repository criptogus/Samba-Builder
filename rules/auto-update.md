...
- Auto-update clients must only receive releases accepted by the provenance verifier in `samba-cloud/apps/api/src/app/v1/update/release_trust.ts`; a GitHub release and its asset digests are not sufficient trust signals by themselves.
...
- The Windows `Squirrel ... CheckForUpdate` → `(404) Not Found` block in bug-report updater logs is a widespread background signal, not the reporter's problem: 23 empty session reports (May–Sep 2026, all on old versions) carried it, and the triage bot used to title them after it. Tracked in samba-sh/samba#4466; the triage playbook's `empty-report` entry tells the bot to ignore that section for titles and assessments.
